...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		// Stage
		var bg:BGSprite = new BGSprite('customizegameplaymenu/sky', -600, -200, 0.2, 0.2);
		add(bg);

		var hills:BGSprite = new BGSprite('customizegameplaymenu/hills', -225, -125, 0.5, 0.5);
		hills.setGraphicSize(Std.int(hills.width * 1.25));
		hills.updateHitbox();
		add(hills);

		var gate:BGSprite = new BGSprite('customizegameplaymenu/gate', -226, -125, 0.6, 0.6);
		gate.setGraphicSize(Std.int(gate.width * 1.2));
		gate.updateHitbox();
		add(gate);

		var grass:BGSprite = new BGSprite('customizegameplaymenu/grass', -225, -125, 0.9, 0.9);
		grass.setGraphicSize(Std.int(grass.width * 1.2));
		grass.updateHitbox();
		add(grass);

		// Characters
		gf = new Character(400, 130, 'DONTDELETEgf');
		...
		boyfriend.y += boyfriend.positionArray[1];
     	dad = new Character(120, 75, 'DONTDELETEdad', true); // why is this mf not dancing
		...
		poophud = new FlxSprite(-120, -120).loadGraphic(Paths.image('customizegameplaymenu/hud', 'preload'));
		poophud.scrollFactor.set();
		poophud.antialiasing = ClientPrefs.globalAntialiasing;
		poophud.cameras = [camHUD];
		poophud.screenCenter(X);
		poophud.screenCenter(Y);
		add(poophud);
		...
		changeModeText.setFormat(Paths.font("comic-sans.ttf"), 32, FlxColor.WHITE, CENTER);
	...
	function createTexts()
	{
		for (i in 0...4)
		{
			...
			text.setFormat(Paths.font("comic-sans.ttf"), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
