...
class PurResetScoreSubState extends MusicBeatSubstate
{
	var bg:FlxSprite;
	...
	public function new(song:String, difficulty:Int, character:String, week:Int = -1)
	{
		...
		if(week > -1) {
			name = PurWeekData.weeksLoaded.get(PurWeekData.weeksList[week]).weekName;
		}
		...
		if(controls.BACK) {
			...
			if(onYes) {
				if(week == -1) {
					...
					Highscore.resetWeek(PurWeekData.weeksList[week], difficulty);
				}
			}
			FlxG.sound.play(Paths.sound('cancelMenu'), 1);
			#if android
